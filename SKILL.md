---
name: innovative-app-generator
description: "一键生成可申请中国软件著作权的创新软件项目，并连续完成项目本体、截图、申请信息、代码材料和手册产物。适用于用户只提供软件名称和可选要求，希望不中断地直接产出完整项目与五个标准交付物的场景。"
---

# 创新应用一键生成器

## 概述

这个 skill 把三类能力合并成一条不中断的工作流：

- 创新项目生成
- 软著手册生成
- 软著代码材料生成

用户输入应尽量简单：

- 必填：软件名称
- 可选：补充要求

输出必须完整：

- 项目本体
- 项目截图
- 一份申请信息 TXT
- 一份完整代码 Word
- 一份前 30 页加后 30 页的代码 PDF
- 一份手册 DOCX
- 一份手册 PDF

最终打包交付目录内只允许出现这 5 个面向用户的文件：

- `XXX.txt`
- `XXX代码（完整版）.docx`
- `XXX代码（60页）.pdf`
- `XXX手册.docx`
- `XXX手册.pdf`

审计截图、文本抽取文件、HTML 中间件、重复命名文件、校验报告、临时文件，必须放在其他目录，不能混进最终打包目录。

## 何时使用

适用于这类请求：

- “帮我做一个可申请软著的创新系统，名字叫 XXX”
- “输入名称和要求，一键生成项目和全部软著材料”
- “不要中间停，直接做完项目、截图、说明书和代码材料”

不适用于只想要单一产物的情况，例如：

- 只要代码材料
- 只要 HTML 手册
- 只要截图
- 只要项目脚手架

这些场景应改用更小的专用 skill。

## 不中断规则

这个 skill 明确要求不中断执行：

- 用户给出软件名称和可选要求后，不得再为了常规细节停下来确认。
- 细节不全时应做合理默认。
- 只有在缺少关键信息且无法安全执行时，才允许提问。

## 文档质量红线

在编写软著手册或其他使用文档时：

- 绝对禁止用硬编码重复内容冒充模块解释。
- 不得把同一段泛化话术换个标题后贴到多个模块里，假装是“逐行解释”“输入输出说明”“数据流”“特殊处理”。
- 每个模块的解释必须对应它自己的真实代码路径、条件分支、阈值、字段映射、返回值和界面行为。
- 如果插入代码引用后，整段内容仍然像可复用模板，则该段不合格，必须重写。
- 不得写“本表来自真实项目”“不是模板”“不是复制来的”这类刻意自证清白的话。
- 不得出现“降低示例数据造假的风险”“避免造假”“防止造假”“伪造材料”这类会暴露错误思路的描述。
- 不得出现“用于证明参数链路真实存在”“可被记录和手册说明”“写进正式资料目录”“避免中间成果与最终交付物混放”这类暴露自动化生成思路的句子。
- 绝对不得出现任何“面向材料生成”“为了材料输出”“用于截图证明”“用于证明界面存在”“为了手册记录”“为了留痕展示”这类面向材料生成或截图证明的叙述。
- `截图`、`软著` 这两个词属于最终成品正文中的绝对违禁词；若最终 TXT、手册或其他交付正文出现这两个词，直接判定任务失败。
- 核心代码节选必须短而准，不得在手册里塞连续两三页的大段代码。
- 手册和代码文档必须统一使用一种正文字体，不得混用花体、斜体或明显不同的代码字体。
- 最终手册与信息单必须经过重复段落硬检测；只要发现同一自然段或长句被原样重复粘贴到多个位置，直接判定任务未完成。
- 对“造假”“伪造”“生成 AI 虚拟”“AI 虚拟”“虚构数据”“伪造材料”等高风险措辞，必须触发一次硬报警；报警后不得直接忽略，必须由 agent 人工复核相关上下文，确认无问题后才能关闭报警并继续交付。

## 原创性红线

这个 skill 严禁模仿、轻度换皮或结构性复用既有项目：

- 不得直接克隆一个已有业务系统，只改名称、颜色、图标或字段文案。
- 不得复用另一个已生成项目的主骨架，导致模块结构、代码组织、页面构成或业务逻辑骨架基本不变。
- 不得把“同一套后台壳、只换行业文案”当作创新项目。
- 不得以任何已有本地项目、历史生成项目或现成业务项目作为实现底稿、主骨架或代码起点。
- 绝对禁止使用通用后台骨架式生成器、通用管理后台模板、通用中台壳、通用仪表盘壳或任何同类通用项目骨架作为生成基础。
- 严禁模仿、轻度换皮或结构性复用既有项目；即使业务名称不同，只要信息架构、界面骨架、模块布局或代码组织明显来自既有项目，也视为违规。
- 生成系统必须具备与目标场景强相关的规则、算法、数据结构和交互流程。

一旦发现当前实现路径与已有项目相似度过高：

- 该项目直接判定无效。
- 只要发现是从已有项目出发改造，而不是从零构建当前目标系统，也直接判定无效。
- 必须废弃当前方案并重做。
- 不得继续包装、润色、出截图或出软著材料。

原创性是硬门禁，不是靠文档修饰解决的问题。

## 默认技术基线

除非用户明确指定其他技术路线，否则默认优先按桌面应用实现，而不是默认做 Web：

- 后端 / 主程序语言：Java 21
- 默认运行形态：桌面应用
- 默认桌面技术：Java Swing 或 JavaFX
- 若用户明确要求同时有前端页面或 Web 配套能力，可再补充 JavaScript / TypeScript 界面层
- 视觉要求：精致、明确、有设计感，不能是随手拼的通用后台
- 文档范围：整个项目，不是只看后端

Web 不是被禁止，但只有在以下情况之一成立时才优先采用：

- 用户明确要求做 Web
- 业务场景天然更适合浏览器访问
- Web 形态更有利于真实展示流程，且不会导致界面显得假、截图不完整或操作记录失真

桌面应用默认要求全中文、窗口结构稳定、控件不重叠、截图真实可审阅。

桌面应用初始打开时还应满足：

- 默认窗口大小尽量控制在常规屏幕可视区域的三分之二以内
- 不得默认以过大的窗口尺寸启动，导致任务栏弹出、窗口超出屏幕、底部或右侧内容被遮挡
- 若当前窗口尺寸会让界面无法在屏幕内完整显示，应先调整窗口设计与默认尺寸，再进入截图流程

## UI 参考图一致性规则

当用户提供参考截图或明确指定目标 UI 示例时：

- 最终界面必须在整体结构、布局节奏、间距、卡片位置、顶栏与侧栏构成、字体层级和视觉密度上高保真贴近参考图。
- 不能只做到“气质差不多”或“颜色像”。
- 如果结果明显偏离参考图，必须继续重做，直到它能被识别为同一目标构图改成对应业务领域后的版本。
- 截图质检时，必须把产出界面和参考图直接对比，而不是只做一般 UI 检查。

## AI 概念参考图规则

在正式实现项目前，必须先生成 AI 概念参考图，用于锁定：

- 页面整体构图
- 模块分区关系
- 视觉风格方向
- 业务内容如何映射到界面卡片与区域

AI 参考图不应强行把所有页面挤进一张图里。应根据页面数量和复杂度，生成多张参考图，分别对应首页、关键一级页面、重点详情页或特殊业务流程页。

AI 概念参考图提示词不得出现“一个图片展示全部界面设计”“一张图包含所有页面”“整套系统所有页面总览”这类表达。

必须遵守以下规则：

- 一张 AI 参考图只对应一个界面或一个独立页面状态
- 可以先生成主页参考图，再分别生成列表页、详情页、导入页、结果页、设置页等参考图
- 如果某个流程包含弹窗或关键二级状态，也应把该弹窗或该状态单独作为一张参考图处理
- 不得为了省事把多个一级页面、多个模块主界面或整套系统总览强行塞进同一张图里
- 后续正式界面实现时，也应按“一张参考图对应一个页面”的方式逐页对齐

AI 参考图生成提示词里必须明确加入以下倾向：

- 尽量不要使用暗色风格
- 优先使用浅色、白底或明亮中性色界面
- 保持专业、真实、可落地的企业级桌面软件视觉
- 避免为了“高级感”强行做深色玻璃风、霓虹风或过重的炫光效果

如果生成出的 AI 参考图存在以下问题，允许先重生成，再进入正式实现：

- 结构混乱，不利于落地
- 视觉过丑或明显不适合目标软件
- 与用户要求或参考截图偏离过大
- 页面信息塞得过满，无法形成清晰单页设计

一旦确定使用某组 AI 参考图，后续正式界面必须同时对齐：

- 用户给定的参考图
- 已确认的 AI 概念参考图

不得跳过这一步直接编码。确认参考图之后，最终界面必须尽可能与参考图完全一致，而不是只保留一个大概风格。

这一步必须真实执行，而不能只停留在规则文字上：

- 必须先实际生成 AI 概念参考图文件，再进入正式界面实现
- 不得只在说明里声称“参考了 AI 图”，但实际上没有生成任何参考图
- 若没有实际执行 AI 概念参考图流程，整个项目直接判定未完成
- 若生成了参考图但后续界面明显未按图实现，也视为未通过

## 单屏可读性规则

前端界面应尽量控制在单个常规屏幕首屏即可完整阅读的范围内：

- 一级页面不要做成过长的瀑布流式堆叠页面
- 主要信息、主要操作按钮、核心图表和关键表格区域应尽量在一个屏幕内完成展示
- 若业务内容较多，应优先通过分栏、分页、选项卡、折叠区或二级页面拆分，而不是无限向下堆内容
- 截图目标页面应尽量做到“单屏即可看清完整核心界面”，避免必须依赖超长整页截图才能说明功能
- 若某页明显过长，导致首屏无法体现完整核心结构，则应先调整页面布局，再进入截图阶段

## 项目内必须生成的内容

在项目目录内，始终创建这些内容：

- `docs/需求文档.md`
- `docs/TODO.md`
- `softcopyright-manifest.json`（由 skill 自动创建并持续补齐）
- 可运行的项目源码
- 截图输出目录
- `软件著作权申请资料/正式资料`

`softcopyright-manifest.json` 是内部契约文件，用于驱动手册和打包脚本。用户不应被要求手工编写或手工补齐它。

## Manifest 规则

实现过程中必须自动创建并维护 `softcopyright-manifest.json`。

manifest 至少要包含：

- 软件名称
- 版本号
- 著作权人或归属口径
- 开发目的
- 所属行业
- 用户群体
- 环境字段
- 模块清单
- 截图映射
- 每个核心模块对应的代码引用
- 导出说明
- 测试说明
- 维护说明

使用 [references/manifest-schema.md](references/manifest-schema.md) 和 [assets/manifest-template.json](assets/manifest-template.json) 作为内部约束。若 manifest 缺失或字段不完整，skill 必须自动从真实代码和项目结构中回填，而不是要求用户手动处理。

## 工作流

### 1. 创建项目

除非用户指定路径，否则在当前工作区内按软件名称创建新项目目录。

立即生成：

- `docs/需求文档.md`
- `docs/TODO.md`

在正式编写项目代码前，必须先执行 AI 概念参考图流程：

- 至少先生成一张主页参考图
- 再按需要生成列表页、详情页、导入页、结果页、设置页等参考图
- 参考图文件必须实际落盘，供后续界面实现与截图质检使用
- 未完成这一步，不得进入正式项目实现

项目必须具备可用于后续软著申报的创新性：

- 不能只是一个薄薄的 CRUD 壳
- 至少包含 3 个与业务场景强相关的规则、算法或协同机制
- UI 必须完整到足以支持截图和操作流说明
- 不得直接仿造本地已有项目

### 2. 完整实现项目

在同一轮内把项目做成可运行版本。

实现要求：

- 所有主要页面和一级模块都存在
- 项目能本地运行
- 页面文案和导航稳定
- 核心模块有清晰可引用的源码文件
- 项目本体源码总量默认必须达到 7000 行以上；若不足，必须继续增加真实业务模块、算法逻辑、分析能力或高阶功能，不能靠辅助脚本、模板文件、配置堆砌或文档生成代码凑行数
- 为了同时提高创新性与源码规模，默认应强烈优先加入更高阶的真实功能，例如机器学习、智能规则引擎、优化调度、时序预测、异常检测、知识推理、图模型分析等；只要场景合理，应优先用这些能力扩展系统，而不是停留在薄层 CRUD
- 这些高阶功能必须与目标业务直接相关，能够在界面、流程、结果展示和手册中被真实说明；不得为了凑代码量而硬塞无关算法模块
- 首次启动时默认没有业务数据，不得为了演示预置真实业务记录
- 若系统涉及数据录入或批量处理，必须提供“导入数据”能力
- 若提供导入能力，必须同时提供“导出模板”能力，便于用户先理解填报格式
- 若提供导入能力，还必须同步生成一份可直接测试导入的数据文件
- 不得用“生成示例数据”替代“导入数据”；若确有测试数据准备步骤，也应围绕导入模板和测试数据文件展开
- 允许提供默认可导入的示例数据文件，但该示例数据必须来自独立测试数据文件或导入包，绝对禁止把业务数据硬编码在界面初始化或主程序默认状态里
- 程序源码文件命名应保持英文或规范技术命名，不要把版本号直接拼进程序文件名
- 面向用户交付的测试数据文件、模板文件和导出样例可采用中文命名，便于软著材料审阅

实现过程中必须同步更新 `softcopyright-manifest.json`，保证它反映当前真实代码，而不是先写愿景再慢慢对齐。

同时在实现阶段先做一次原创性检查：

- 将拟定架构和核心模块与附近已有项目比较
- 若相似度高，立刻放弃当前方案并重做
- 只有通过这道原创性门禁后，才能继续做截图和软著材料
- 在原创性检查中，必须额外确认当前实现不是由通用后台骨架式生成器、通用管理壳或历史项目骨架演化而来；只要是这种来源，直接判定无效并重做

项目功能完成后，还要做第二次原创性复检：

- 对照已有本地项目和开发过程中的明显参考实现
- 检查目录结构、模块边界、路由布局、页面构成、命名模式、数据模型和核心规则逻辑
- 不能把简单改名视为“已差异化”
- 若原创性仍弱，相似度仍高，必须继续修改后再进入截图与最终材料阶段

### 3. 提前保证可截图

正式截图前必须先确保：

- 本地应用能正常运行
- 主要界面都能进入
- 布局重叠、乱码、空白状态都已修复

优先做稳定的菜单驱动型页面，这样每个主要模块都能清晰截图。

### 4. 生成并检查截图

必须为所有主要界面截图。

截图默认必须以软件界面的整页截图为准：

- 对主页面、一二级功能页面、详情页、配置页、结果页，默认都要截到完整页面，而不是只截局部区域
- 只有当目标本身是独立弹窗、独立对话框、独立表格区或独立图表区时，才允许单独截该完整区域
- 不得为了掩盖布局问题、内容过少或内容过假而故意只截某个小角落
- 如果一个页面长到无法真实、清晰地做整页截图，优先说明页面设计有问题，应先重构页面再截图

优先使用应用内浏览器或 Playwright / 浏览器自动化。至少覆盖：

- 首页 / 总览页
- 每个一级模块
- 重要二级视图、详情区、弹窗或特殊流程
- 导入模板、导入数据、导出结果、参数调整、保存确认、异常提示等关键操作状态

截图原则不是“每个模块一张图就结束”，而是“每个关键步骤和关键界面状态都要有记录”：

- 能够共用同一页面截图的连续小步骤可以合并，但前提是这张图能把该组步骤的界面状态完整说明白
- 只要页面状态发生明显变化，例如弹窗打开、数据导入完成、参数保存成功、结果区域刷新、导出成功提示出现，就应新增截图
- 必须覆盖二级菜单、详情窗口、导入流程、模板下载流程、结果导出流程，不能只截首页和一级菜单

截图必须保存到项目本地目录，并自动写入 `softcopyright-manifest.json`。

截图完成后，必须逐张检查：

- 每张主要截图都要单独看
- 检查字段是否缺失、遮挡、重叠
- 检查截图是否截到了完整的目标界面，而不是残缺局部
- 检查中文是否乱码、是否出现黑框字形
- 检查截图本身是否足够清晰，文字、图标、图表和按钮标签必须可辨认，不能模糊、压缩过度或分辨率过低
- 检查截图呈现出的布局、卡片层级、留白节奏、字体层级和视觉密度是否仍与 AI 概念参考图一致
- 检查截图中不得出现应用界面以外的内容，尤其是桌面任务栏、系统托盘、桌面背景、窗口边界外杂项内容
- 只要截图有布局问题、内容缺失、乱码或黑框，就必须修复项目并重截
- 凡是没有显示完整页面、完整弹窗、完整表格区域或完整图表区域的截图，都视为无效
- 凡是本应整页截图却只截局部内容的截图，也视为无效
- 如果截图缺下方、缺右侧、按钮没截全、标签被切断，也视为无效
- 如果截图中出现底部任务栏、桌面背景或任何应用外内容，直接判定无效，必须重新截图
- 如果窗口过大导致任务栏弹出、屏幕无法完整容纳窗口、下方或右侧被遮挡，这不是小问题而是直接打回项，必须先修改默认窗口尺寸和布局再重截
- 如果截图清晰度不足，导致正文说明无法依图审阅，也视为无效
- 如果截图成品与 AI 概念参考图在风格和构图上明显偏离，也视为未通过截图质检
- 只有当截图证明“整块可审查界面状态都可见”时，截图任务才算完成
- 截图必须来自真实运行中的桌面程序窗口抓屏或真实程序界面抓取，绝对禁止用画图、PIL、ImageDraw、示意图拼接、静态海报式排版或任何伪界面图片冒充软件截图
- 若截图链路使用的是绘图脚本而不是真实程序窗口抓屏，则整个项目直接判定无效，必须重做截图流程

截图数量也有硬门槛：

- 最终可进入手册的有效截图不得少于 15 张
- 这 15 张以上截图必须覆盖不同功能、不同页面状态或不同操作步骤，不能靠同一页面微小变化凑数
- 如果做不到至少 15 张彼此有明确功能差异的有效截图，说明软件功能设计或流程层次不够，必须先重做软件结构，再继续截图

### 5. 生成软著打包产物

项目和截图准备好后，执行：

```powershell
python "C:\Users\Administrator\.codex\skills\innovative-app-generator\scripts\generate_bundle.py" `
  --project "<project-root>"
```

这一步会产出：

- 申请信息 TXT
- 完整代码 DOCX
- 前 30 页加后 30 页代码 PDF
- 手册 DOCX
- 手册 PDF

这 5 个文件必须集中放进一个单独的最终打包目录。不得和审计图、截图复核目录、文本抽取文件、HTML 中间件或重复命名文件混放。

在最终打包目录整理完成后，还必须再额外生成一个压缩包：

- 压缩包内只允许包含这 5 个最终交付文件
- 不得把审计图、HTML 中间件、校验报告或临时文件打进压缩包
- 压缩包文件名必须直接等于项目中文名称
- 例如项目名称为“智慧校园管理系统”，则压缩包名称应为 `智慧校园管理系统.zip`

总控入口必须做到只传 `--project` 就能跑通。若要求用户手工写或手工改 manifest，就违背了一键化约定。

### 6. 最终质检

结束前必须完成以下检查：

- 确认应用可运行
- 确认截图已生成
- 确认截图已逐张检查
- 确认手册 DOCX 已生成
- 如果两个模块的解释除了标题和几个名词外几乎一样，手册直接无效，必须重写
- 如果逐行解释、输入输出说明、数据流说明、特殊处理说明是硬编码重复模板，任务直接不算完成
- 必须检查最终生成的 TXT、DOCX、PDF 成品本身，而不只是检查 manifest 或中间 markdown
- 必须对最终手册和信息单做逐页视觉检查，关注裁切、`??`、替代字符、黑框、乱码等问题
- 必须把最终手册逐页渲染成截图，并按页检查这些截图
- 不能只依赖文本抽取或 XML 检查，必须看最终渲染后的页面图
- 只要任意一页手册截图存在问题，整份手册都不可交付，必须修复后重生
- 必须检查手册中所有应填写字段是否存在空白、留空单元格、空白占位或明显缺项；只要发现空白字段，整份手册直接判定有问题，必须修复后重生
- 若最终成品中仍有 `??`、乱码、陈旧模板句或截图残缺，任务不算完成
- 若最终手册或信息单存在重复段落、重复长句或复制型说明块，任务不算完成
- 若最终文档中出现“造假”“伪造材料”“降低示例数据造假的风险”等措辞，任务直接失败
- 对“造假”“伪造”“生成 AI 虚拟”“AI 虚拟”“虚构数据”等高风险词，至少要触发一次报警并完成人工复核；在 agent 明确确认前，不得把报警视为已处理
- 确认手册 PDF 已生成（环境支持时）
- 确认 TXT 文件存在，且符合 `【字段】内容` 的严格格式
- 确认完整代码 DOCX 已生成
- 确认前后 30 页代码 PDF 已生成（环境支持时）
- 确认最终五个文件已经被打进一个单独压缩包
- 确认压缩包名称与项目名称完全一致
- 确认压缩包内只包含五个最终交付文件
- 必须把最终手册整份从头到尾人工读完，不能只读片段
- 必须按交付物标准检查手册的重复表达、占位残留、过期模板句、模块解释不一致、代码与文档不匹配
- 不得保留“这是真的”“不是模板”之类表演式措辞
- 每段代码节选都要简洁，不能膨胀成大段连续堆砌
- 代码材料生成后，必须核对代码量和导出页数是否真实匹配，不能想当然
- 只有在“完整读完并检查完成品”之后，才允许判定任务完成
- 确认名称和版本号在所有产物里一致
- 再次确认项目不属于高相似度衍生物
- 若最终项目原创性仍不足，必须继续改核心模块、流程和代码结构

还必须做代码文档截图抽检：

- 对生成出的代码文档截图抽检
- 至少检查以下文档的首页、中间页、末页：
  - 完整代码 Word
  - 前 30 页加后 30 页代码 PDF
- 检查内容可见性、页码、页内容连续性、中文可读性，以及每页是否保持预期的 50 行内容布局
- 若代码文档出现裁切、乱码、分页错误、不可读文本或代码量与页数不匹配，必须重导出

全流程必须强制使用 UTF-8：

- skill 创建或修改的源码文件必须使用 UTF-8
- 生成的 manifest、markdown、HTML、TXT 等文本产物必须使用 UTF-8
- 文件生成后必须检查中文是否乱码或出现黑框字形
- 发现编码损坏时，不得继续交付，必须修复并重生

## 输出文件规范

默认输出目录：

- `<project>/软件著作权申请资料/正式资料`

最终标准文件名：

- `<系统名称>.txt`
- `<系统名称>代码.docx`
- `<系统名称>代码.pdf`
- `<系统名称>手册.docx`
- `<系统名称>手册.pdf`

最终还必须额外输出：

- `<系统名称>.zip`

该压缩包必须只包含以上 5 个文件，不得混入其他文件或目录。

当总代码页数超过 60 页时：

- Word 代码文件名必须显式包含 `（完整版）`
- PDF 代码文件名必须显式包含 `（60页）`

所有这 5 个交付物必须放在同一个最终输出目录里，不能把最新的 TXT、代码文件和手册文件散落到各个验证目录。

### 代码文档规则

- Word 代码文档是完整代码文档
- PDF 代码文档只保留完整代码文档的前 30 页和后 30 页
- 当总代码页数超过 60 页时，PDF 必须严格等于 60 页
- 代码文档必须使用竖版
- 页边距必须为：上 1.5 cm、下 1.5 cm、左 2 cm、右 2 cm
- 代码文档不得带封面页或结尾说明页
- 页眉应显示 `<系统名称>代码`
- 页脚应显示页码
- 代码内容必须连续，不得为了装饰插入分隔页
- 每个实际物理页必须校准为 50 行内容
- 代码量统计必须只计算项目本体源码
- 生成辅助脚本、打包脚本、截图工具、文档生成器、依赖锁文件、构建配置、`softcopyright-manifest.json`、软著输出目录等都不得计入源程序量
- 这些辅助或打包文件也不得进入代码文档

### 申请信息 TXT 规则

- 严格使用 `【字段】内容` 格式
- 下列字段全部是强制字段，缺一不可：
  - `【软件全称】`
  - `【版本号】`
  - `【软件分类】`
  - `【开发的硬件环境】`
  - `【运行的硬件环境】`
  - `【开发该软件的操作系统】`
  - `【软件开发环境/开发工具】`
  - `【该软件的运行平台/操作系统】`
  - `【软件运行支撑环境/支持软件】`
  - `【编程语言】`
  - `【源程序量】`
  - `【开发目的】`
  - `【面向领域/行业】`
  - `【软件的主要功能】`
  - `【软件的技术特点】`
- 需要限制长度的短字段必须控制在 50 个汉字内
- `软件的主要功能` 必须控制在 700 到 1200 个汉字之间，且不能靠重复模板话术凑字数
- `软件的技术特点` 必须采用“技术特点分类；100 字内技术特点说明”的格式
- 技术特点分类必须明确写出 1 到 2 个分类，例如：`医疗软件、人工智能软件`
- 技术特点说明必须与真实源码、真实功能和后续手册内容一致，不能写成空泛概念词堆砌

### 手册规则

- 手册正文中不要写 `版本号`
- 手册正文中不要写 `总代码行数`
- 手册全文字体必须统一，不得混入其他代码字体或斜体风格
- 手册必须带页码，页码必须出现在最终 DOCX/PDF 成品中
- 手册中的环境表、字段表、功能说明表、技术特点说明等应填内容不得留空；若存在空白字段、空白内容格或未填写的占位区域，手册直接不合格
- 手册中的截图必须尽量多且尽量细，目标是把关键操作一步步记录清楚，而不是只靠少量大图概括
- 若系统属于桌面应用，截图应体现真实窗口、真实对话框、真实按钮状态和真实导入导出流程
- 手册中应显式展示“软件技术特点分类”，再接技术特点说明，不能把分类信息遗漏掉

## 辅助脚本说明

### `scripts/generate_bundle.py`

统一产出整套软著交付物。

### `scripts/generate_html_manual_from_manifest.py`

根据 manifest 和真实代码生成手册，并输出 DOCX 与 PDF。

### `scripts/generate_code_pages.py`

生成完整代码 Word、前后 30 页代码 PDF，以及申请信息 TXT。

## 备注

- 这是一个面向“整个项目”的 skill。若项目同时包含前端和后端，不得偷偷缩成只看 Java。
- 手册和代码材料必须引用真实代码、真实截图和真实模块。
- 除非用户明确要求，否则最终材料里不得出现“生成日期”“自动生成”等机器口吻。
- 若原创性检查失败，正确做法是废弃项目并重做，而不是靠修文案挽救。
- 原创性必须检查两次：一次在设计 / 实现方向确定时，一次在项目完成后。
- 截图检查、代码文档截图抽检和 UTF-8 编码检查，都是强制完成门禁。
- 当用户要求五个标准交付物时，不得把 HTML 中间件当作最终命名产物留在交付目录中。
